 ...
 */
export function CaregiverAppointmentCard({ data }: CaregiverAppointmentCardProps) {
  ...
  const getStylesByDuration = () => {
    if (isLessThanTwoHours) {
      return {
        ...
        title: "text-[1.1rem] font-bold truncate",
        button: "bg-[#FFEEE5] text-[#FF6619] font-bold text-xs px-1 py-0.5 border-4 border-[#FFAA80] hidden",
      }
    }
    if (isExactlyTwoHours) {
      return {
        ...
        title: "text-[1rem] font-bold truncate",
    ...
    if (duration <= 4) {
      return {
        ...
        title: "text-[1rem] font-bold truncate max-w-full",
    ...
    return {
      ...
      content: "flex-col items-start gap-1",
      title: "text-[1rem] font-bold text-[#FA5300] truncate max-w-full",
  ...
  return (
    <div
      className={` absolute left-0 right-0 rounded-lg p-2 
        ...
        hover:shadow-lg hover:z-10 cursor-pointer flex flex-col justify-between ${styles.wrapper}`}
      style={{
        top: `${top}px`,
        height: `${height}px`,
    ...
    >
      <div className={`flex ${styles.content}`}>
        ...
        <div className={`flex ${isExactlyTwoHours ? "flex-col justify-center" : "flex-col"} ${styles.container || ""}`}>
          ...
          {!isLessThanTwoHours && styles.location && (
            <span className={` -mt-1 ${styles.location}`} title={data.client.location}>
      ...
      {(!isLessThanTwoHours || height > 40) && (
        <div className={ `text-small-text  px-2 py-2 rounded-md text-center ${styles.button} w-full`}>
