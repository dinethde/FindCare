...
export function SidebarCg() {
    return (
        <div className=".sidebar-color bg-[#F1F3F4] flex flex-col justify-between w-full text-h6
         max-w-[15ch] p-4 rounded-regular">
            <div className="flex flex-col gap-8 text-regular-text">
                {/* Upper */}
                <div className="space-y-1">
                    <h2 className="text-h6 ">{profile.name}</h2>
                    ...
                    {navigation.map((item) => (
                        <Link
                            ...
                            className={`flex items-center gap-2 ${
                                item.isActive ? "text-[#ff4b00] font-medium" : "text-[#424957] hover:text-[#ff4b00]"
                            }`}
                        >
            ...
            <div className={`space-y-3 text-regular-text text-neutral-10 w-full `}>
                ...
                <div className="flex items-center space-x-2 ">
                    <Image className="h-7 w-7 rounded-[50%]" src={photo} alt={profile.name} width={20}
                           height={20}/>
