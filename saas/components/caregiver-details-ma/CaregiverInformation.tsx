...
} from "@/types/caregiver-details-ma/caregiver";
...
const CaregiverInformation = () => {
  ...
  const [caregiverData, setCaregiverData] =
    useState<CaregiverData>(initialData);
  ...
  const [formValues, setFormValues] =
    useState<Partial<CaregiverData>>(caregiverData);
  ...
  const handleInputChange = (
    field: keyof CaregiverData,
    value: string | number | FeatureItem[] | CertificationItemType[]
  ) => {
    setFormValues((prev) => ({
      ...prev,
      [field]: value,
  ...
  return (
    // Main container for caregiver information
    <div className="bg-white rounded-lg shadow-sm border p-5 flex flex-col gap-5">
      {/* Header with back button and edit button */}
      <div className="flex items-center justify-between ">
        <div className="flex justify-center items-center gap-2">
          <Link href="/tenant/1/mobileapp" className="text-neutral-9">
            <ArrowLeft size={20} />
          </Link>
          <div className="flex flex-col items-start gap-1">
            <h1 className="text-h6 text-neutral-10">
              {caregiverData.name} Information
            </h1>
            <p className="text-neutral-7">{caregiverData.id}</p>
          </div>
        </div>
        <button className="text-support-colors-color1" onClick={toggleEditMode}>
          <Edit size={25} />
        </button>
      </div>
      ...
      <div className="flex flex-col gap-8">
          ...
          {isEditing ? (
            <>
              <EditableField
                ...
                action={
                  <button className="text-tagline text-support-colors-color1">
                    Find Care
                  </button>
                }
              />
              ...
              <InfoField
                label="Caregiver Id"
                value={caregiverData.caregiverId}
              />
          ...
          {isEditing ? (
            <>
              ...
              <EditableField
                ...
                onChange={(value) =>
                  handleInputChange("totalClients", Number.parseInt(value) || 0)
                }
                type="number"
              />
              ...
              <EditableField
                ...
                onChange={(value) =>
                  handleInputChange("clientSatisfactionRate", value)
                }
              />
            </>
          ) : (
            <>
              ...
              <InfoField
                label="Total clients"
                value={caregiverData.totalClients.toString()}
              />

              <InfoField
                label="Years of excellence"
                value={caregiverData.yearsOfExcellence.toString()}
              />
              ...
              <InfoField
                label="Client Satisfaction Rate"
                value={caregiverData.clientSatisfactionRate}
              />
            </>
          )}
        </FieldGroup>
        ...
        <FieldGroup>
          <FeatureSection
            ...
            selectedItems={
              isEditing
                ? formValues.caregiverSpecificity || []
                : caregiverData.caregiverSpecificity
            }
            allItems={initialData.availableFeatures.caregiverSpecificity}
            onSelectionChange={(items) =>
              updateSelectedFeatures("caregiverSpecificity", items)
            }
            isEditing={isEditing}
          />
        </FieldGroup>
          ...
          <FeatureSection
            ...
            selectedItems={
              isEditing
                ...
                : caregiverData.keyFeatures
            }
            allItems={initialData.availableFeatures.keyFeatures}
            onSelectionChange={(items) =>
              updateSelectedFeatures("keyFeatures", items)
            }
            isEditing={isEditing}
          />
        </FieldGroup>
        ...
        <FieldGroup>
          <FeatureSection
            ...
            selectedItems={
              isEditing
                ? formValues.generalFeatures || []
                : caregiverData.generalFeatures
            }
            allItems={initialData.availableFeatures.generalFeatures}
            onSelectionChange={(items) =>
              updateSelectedFeatures("generalFeatures", items)
            }
            isEditing={isEditing}
          />
        </FieldGroup>
